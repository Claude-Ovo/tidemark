...
92. **重心回归修复复验（`d703fc0`，Claude 实机复验）—— 通过，三条全部收口**：`npm test` exit 0，判别 E1-E10 全绿。

    **① 对齐回归 —— 已修**。实测视口 `1199×1008`（正是上一轮暴露脱节的那个尺寸）：侧栏与内容框现在同一顶边起始，不再读作两个页面。采用 (b) 方案是对的——撤掉 `.workspace` 的居中与 min-height、内容顶对齐、底部 chrome 贴视口底，这套没有任何随视口高度变化的相对位移，所以它在任意高度都成立，不需要靠"再测两个尺寸"来保证。这比 (a) 的整体居中稳，选得对。

    **③ 标签 —— 已改**。实机确认 `NO VISIBLE SPREAD`，不再把舍入结果写成精确断言。E10 的判别描述也同步改成 `visually uniform Tide values collapse only at the displayed precision`，措辞比原来的 `equal-looking` 更准——"视觉上一致"是可判定的，"看起来相等"是主观的。
