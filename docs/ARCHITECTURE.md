...
| **证据前端**（`web/evidence.html`，React + DOM/CSS，无 WebGL；提交主入口候选，静态包已构建、CloudFront 切换待部署验收） | 保留强度账本（共享基线水平条 + fade 阈值参考线）、持久事件流（remember/recall/agent action/outcome 四源）、单条记忆证据（receipt 分量、plasticity before→after、服务端采样衰减曲线）、trace 与 capability 索引；单一 `selectedEvent → selectedMemoryId → selectedDetail → selectedTrace`，事件与记忆断链时显式标注而非静默展示别条记忆的证据；十步生命周期证明置于 `?demo=judge` 只读闸后 | 演示面（只读 viz 端点，与业务面隔离） |
| **记忆潮池前端**（`web/pool.html`，2D canvas 零依赖；当前公网入口 https://dhgwgra6nycty.cloudfront.net） | 总览模块，仍可直链：粒子=记忆、半径=服务端保留强度、recall 涟漪、outcome 潮痕、`/viz/activity` live 消费环、hover 卡+居中详情 modal、键盘/焦点/reduced-motion 全契约 | 演示面（同上） |
...
- [~] 证据前端（`web/evidence.html`）——数据面、Judge 编排、信息架构与视觉已在本地构建验收；CloudFront 静态包上传与默认入口切换尚待部署验收（频道结论 79/80/82/84）
